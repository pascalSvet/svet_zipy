...
public class Tests_recommendedThumbs_cy_populars extends Tests_recommendedThumbs_MAIN {
	
	private static String thumbnail;
	private static String number;

	public Tests_recommendedThumbs_cy_populars(String thumbnail, String number) {
		super();
	...
	//Test - opening thumbnails in populars (Cypress)
	@Test		
	public  void Tests_cy_popularsThumbs() throws Exception {
		System.out.println("Running test for Cypress site - populars - opening thumbnail #" + number);		

		//open the site
		driver.get(ElementsLogin.Zipy_cy);
		
		//click thumbnail
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(thumbnail))).click();	
		wait.until(ExpectedConditions.not(ExpectedConditions.urlToBe(ElementsLogin.Zipy_cy)));

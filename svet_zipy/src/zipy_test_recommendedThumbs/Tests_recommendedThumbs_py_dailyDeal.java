...
public class Tests_recommendedThumbs_py_dailyDeal extends Tests_recommendedThumbs_MAIN {
	
	private static String thumbnail;
	private static String number;

	public Tests_recommendedThumbs_py_dailyDeal(String thumbnail, String number) {
		super();
	...
	//Test - opening thumbnails in dailyDeal (Portugal)
	@Test		
	public  void Tests_py_dailyDealThumbs() throws Exception {
		System.out.println("Running test for Portugal site - dailyDeal - opening thumbnail #" + number);		

		//open the site
		driver.get(ElementsLogin.Zipy_py);
		
		//click thumbnail
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(thumbnail))).click();		
		wait.until(ExpectedConditions.not(ExpectedConditions.urlToBe(ElementsLogin.Zipy_py)));

...
public class Tests_recommendedThumbs_ro_dailyDeal extends Tests_recommendedThumbs_MAIN {
	
	private static String thumbnail;
	private static String number;

	public Tests_recommendedThumbs_ro_dailyDeal(String thumbnail, String number) {
		super();
	...
	//Test - opening thumbnails in dailyDeal (Romania)
	@Test		
	public  void Tests_ro_dailyDealBar_1() throws Exception {
		System.out.println("Running test for Romania site - dailyDeal - opening thumbnail #" + number);	

		//open the site
		driver.get(ElementsLogin.Zipy_ro);
		
		//click thumbnail
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(thumbnail))).click();		
		wait.until(ExpectedConditions.not(ExpectedConditions.urlToBe(ElementsLogin.Zipy_ro)));
